...
Note that every algebraic torus acting on $X$ may be realised in this way \todo{Referenz, ggf. Example 3.1 in Dolgachev ausarbeiten (S. 38)}.
...
\begin{defi}
	...
	\index{GSigmaHX@$\mathcal{G}(\Sigma_H(X))$}%
	\index{EGSigmaHX@$E_{\mathcal{G}(\Sigma_H(X))}$}%
\end{defi}
...
\begin{algorithm}
	...
	\KwIn{A full dimensional GIT cone $\lambda\in\Sigma_H(X)$}
	\KwOut{All full dimensional GIT cones $\mu\in\Sigma_H(X)$ such that $\lambda\cap\mu$ is a common facet of $\lambda$ and $\mu$}
	\BlankLine
...
\begin{algorithm}
	...
	\While{$\mathcal{F}\neq \emptyset$}{
		...
		\For(\Comment*[f]{by Algorithm~\ref{algo:compute_git_cone_neighbours}}){$\mu$ with $\{\lambda,\mu\}\in E_{\mathcal{G}(\Sigma_H(X))}$}{
			\If{$\mu\notin\mathcal{C}$}{
...
\begin{defi}
	...
	\index{GSSigmaHX@$\mathcal{G}_\mathcal{S}(\Sigma_H(X))$}%
	\index{EGSSigmaHX@$E_{\mathcal{G}_\mathcal{S}(\Sigma_H(X))}$}%
\end{defi}
...
\begin{algorithm}
	...
	\KwIn{a GIT cone orbit $\O\in\faktor{\Sigma_H(X)^{(k)}}{\mathcal{S}}$}
	\KwOut{All GIT cone orbits $\O_\mathcal{N}\in\faktor{\Sigma_H(X)^{(k)}}{\mathcal{S}}$ such that $\{\O, \O_\mathcal{N}\}\in E_{\mathcal{G}_\mathcal{S}(\Sigma_H(X))}$}
	\BlankLine
	$\mathcal{N} \leftarrow \emptyset$\;
	Choose $\lambda$ such that $\O = \mathcal{S}\lambda$\;
	\For(\Comment*[f]{by Algorithm~\ref{algo:compute_git_cone_neighbours}}){$\mu$ such that $\{\lambda,\mu\}\in E_{\mathcal{G}(\Sigma_H(X))}$\label{algo_line:git_cone_orbit_neighbours_loop}}{
...
\begin{algorithm}
	...
	\KwIn{$Q(\gamma),\ \Omega_\ideal^{(k)}$}
	\KwOut{$\textcolor{red}{\faktor{\textcolor{black}{\Sigma_H(X)^{(k)}}}{\mathcal{S}}}$}
	\BlankLine
	\Repeat{$\dim(\lambda(p)) = k$}{
		Choose random point $p\in Q(\gamma)$\;
	}
	...
	\While{$\mathcal{F}\neq \emptyset$}{
		...
		\For(\Comment*[f]{by Algorithm~\textcolor{red}{\ref{algo:git_cone_orbit_neighbours}}}){$\textcolor{red}{\O_\mathcal{N}}$ with $\{\textcolor{red}{\O,\O_\mathcal{N}}\}\in E_{\mathcal{G}_{\textcolor{red}{\mathcal{S}}}(\Sigma_H(X))}$}{
			\If{$\textcolor{red}{\O}\notin\mathcal{C}$}{
...
\todo{Moving Cone invariant under symmetry group}
