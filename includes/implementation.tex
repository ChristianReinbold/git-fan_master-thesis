...
\label{sec:git_cone_data_structure}
...
E_\mathcal{S} &\defeq \left\{\{\mathcal{S}\lambda_1, \mathcal{S}\lambda_2\} \mid \lambda_1, \lambda_2 \in \Lambda(\mathfrak{a},Q)(k) \text{ and }\lambda_1 \cap \lambda_2 \text{ is a facet of both }\lambda_1\text{ and }\lambda_2\right\}.
\end{align*}

Due to \todo{Referenz aus Kap. 3!}, $G_\mathcal{S}$ is connected.

In order to traverse any undirected, connected graph $G=(V,E)$, the following routines have to be provided:
...
\begin{itemize}
	...
	\item A routine \texttt{EQUALITY} for checking equality of two given nodes $v,w\in V$.
\end{itemize}

In order to encode a GIT cone orbit $\mathcal{O}$ in $V_\mathcal{S}$, we consider the GIT cone hashes as in section~\ref{sec:git_cone_data_structure} for all elements in $\mathcal{O}$ and select the lexicographically smallest one. Since this encoding is injective, \texttt{EQUALITY} is implemented by comparing bit strings. \texttt{START} is implemented as in \gitfanlib{}, that is selecting random points $p\in Q(\gamma)$ until the corresponding GIT cone $\lambda(p)$ \todo{Notation konsistent?} has full dimension. It remains to implement \texttt{NEIGHBOURS}.
