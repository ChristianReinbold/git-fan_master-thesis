...
\begin{description}
	\item[\texttt{nodefile}:] A path to a nodefile describing all available computing nodes. In a cluster management and job scheduling system such as \slurm{} \cite{slurm} a nodefile may be obtained by \texttt{generate\_pbs\_nodefile}.
	...
	\item[\texttt{worker-per-node}:] The amount of workers that are set up per node. In our case, we set this value to $16$, matching the amount of cores per node.
\end{description}
