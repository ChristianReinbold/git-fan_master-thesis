...
\begin{itemize}
	...
	\item orbit representatives for the faces of the simplex $\gamma$ under the symmetry group action in case it is provided without simultaneously providing a set of precomputed orbit representatives (see section~\ref{sec:additional_features}).
\end{itemize}

In a next step, the set of either orbit representatives or faces of the simplex $\gamma$, depending on whether a symmetry group action is provided, is partitioned into parts of equal size. A token is generated for every part, which then is processed in parallel, selecting only the \afaces{} with full dimensional image under $Q$. Finally, the reduced parts are merged into a single token that then is passed to the next stage. The Petri net modelling this process is depicted in figure~\ref{net:algorithm_stage_1}.
...
\subsection*{Stage 2: Orbit cones, group action on GIT cones}
